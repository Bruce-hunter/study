...
def main():
    if len(sys.argv) < 2:
        sys.exit('执行方式:python3 Web2.py Framework:app')
    else:
        para = sys.argv[1]
        module_name, app_name = para.split(':')
        m = __import__(module_name)
        # m.app_name这种方式执行的是m模块里面的app_name方法或者是类，并不会识别app_name是变量
        # getattr获取某个对象的某个属性，并返回
